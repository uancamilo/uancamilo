...
async function fetchGraphQL(query, preview = false) {
	try {
		...
		if (!response.ok) {
			console.error("❌ Contentful API Error Response:", text);
...
function extractEntry(fetchResponse) {
	return fetchResponse?.data?.recursosCollection?.items?.[0];
}
...
export async function getEntriesWithSlug() {
	const entries = await fetchGraphQL(`
		query {
			recursosCollection(where: { slug_exists: true }, order: fecha_DESC, limit: 10) {
				items {
					${ENTRIES_GRAPHQL_FIELDS}
				}
			}

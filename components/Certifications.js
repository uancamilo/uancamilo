export default function Certifications({ certifications, contentfulCertifications }) {
  // Use Contentful data if available, otherwise fall back to static data
  const certificationsData = contentfulCertifications && contentfulCertifications.length > 0 
        ...
        {certificationsData.map((cert) => (
          <div key={cert.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-2">
              <h3 className="text-lg font-semibold text-gray-900 pr-2">{cert.name}</h3>
              {cert.url && (
                <a
                  href={cert.url}
                  target="_blank"
            ...
            <div className="text-sm text-gray-600 space-y-1">
              <div className="flex items-center">
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3a2 2 0 012-2h4a2 2 0 012 2v4m-6 0V6a2 2 0 112 0v1m-6 0a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V9a2 2 0 00-2-2m-6 0V7" />
                </svg>
              ...
              {cert.expiryDate && (
                <div className="flex items-center">
                  ...
                  <span>Expira: {new Date(cert.expiryDate).toLocaleDateString()}</span>
                </div>
              )}
